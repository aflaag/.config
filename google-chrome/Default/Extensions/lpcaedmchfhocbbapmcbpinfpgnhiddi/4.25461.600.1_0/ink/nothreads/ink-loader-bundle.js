...
var ASM_CONSTS = {
  317572: ($0, $1, $2, $3) => { return setTimeout( () => window.inkWasmRegistry[$3].dynCalls["vi"]($1, $2), $0); },  
 317657: ($0) => { clearTimeout($0); },  
 317679: ($0, $1) => { window.inkHostRegistry[$1].setTargetFPS($0); },  
 317728: ($0) => { window.inkHostRegistry[$0].bindScreen(); },  
 317773: ($0, $1) => { window.inkHostRegistry[$1].requestImage(UTF8ToString($0)); },  
 317836: ($0, $1, $2, $3) => { window.inkHostRegistry[$3].setCursor($0, $1, $2); },  
 317890: ($0, $1, $2) => { window.inkHostRegistry[$2].navigateToUri(UTF8ToString($0, $1)); },  
 317958: ($0, $1) => { window.inkHostRegistry[$1].setKeyboardMode($0); },  
 318010: ($0, $1, $2) => { window.inkHostRegistry[$2].onAccessibilityData( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 318109: ($0) => { return window.inkHostRegistry[$0].shouldPreloadShaders(); },  
 318171: ($0) => { return window.inkHostRegistry[$0].shouldRegisterDocument(); },  
 318235: ($0, $1, $2, $3, $4, $5) => { const buf = window.inkHostRegistry[$5].renderText( HEAPU8.subarray($0, $0 + $1), $2, $3); HEAP8.set(buf, $4); },  
 318349: ($0, $1, $2, $3, $4, $5) => { const name = window.inkHostRegistry[$0].selectMatchingFont( UTF8ToString($1, $2), $3, !!$4, $5); if (name) { const size = lengthBytesUTF8(name) + 1; const tempBuffer = _malloc(size); stringToUTF8(name, tempBuffer, size); return tempBuffer; } return 0; },  
 318605: ($0, $1, $2) => { window.inkHostRegistry[$2].onSceneChange( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 318698: ($0, $1, $2, $3, $4, $5, $6, $7, $8) => { window.inkHostRegistry[$8].onImageExported( $0, $1, $2, new Uint8ClampedArray(HEAPU8.subarray($3, $3 + $4)), $5, $6, UTF8ToString($7)); },  
 318838: ($0, $1, $2) => { window.inkHostRegistry[$2].onToolEvent( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 318929: ($0, $1) => { window.inkHostRegistry[$1].onSequencePointReached($0); },  
 318988: ($0, $1, $2) => { window.inkHostRegistry[$2].onFlagChanged($0, !!$1); },  
 319044: ($0) => { window.inkHostRegistry[$0].onCameraChanged(); },  
 319094: ($0, $1) => { window.inkHostRegistry[$1].onCameraMovementStateChanged(!!$0); },  
 319161: ($0, $1) => { window.inkHostRegistry[$1].onBlockingStateChanged(!!$0); },  
 319222: ($0) => { window.inkHostRegistry[$0].onInvisibleLayerActionPrevented(); },  
 319288: ($0, $1, $2) => { window.inkHostRegistry[$2].onPdfLoadSuccess( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 319384: ($0) => { window.inkHostRegistry[$0].onPdfLoadPasswordRequired(); },  
 319444: ($0, $1, $2) => { window.inkHostRegistry[$2].onPdfLoadFailure(UTF8ToString($0, $1)); },  
 319515: ($0, $1, $2, $3) => { window.inkHostRegistry[$3].onPdfSaveSuccess( $0, new Uint8Array(HEAPU8.subarray($1, $1 + $2))); },  
 319615: ($0, $1, $2, $3) => { window.inkHostRegistry[$3].onPdfSaveFailure($0, UTF8ToString($1, $2)); },  
 319690: ($0, $1) => { window.inkHostRegistry[$0].onPdfUnsupportedFeature($1); },  
 319750: ($0) => { window.inkHostRegistry[$0].onPdfFormChanged(); },  
 319801: ($0, $1, $2, $3, $4, $5, $6, $7) => { window.inkHostRegistry[$0].onPdfFormFieldFocused( $1, new Uint8Array(HEAPU8.subarray($2, $2 + $3)), $4, !!$5, UTF8ToString($6, $7)); },  
 319938: ($0, $1, $2) => { window.inkHostRegistry[$2].onPdfFormTextFieldFocusChanged( true, UTF8ToString($0, $1)); },  
 320030: ($0) => { window.inkHostRegistry[$0].onPdfFormTextFieldFocusChanged(false); },  
 320100: ($0, $1) => { window.inkHostRegistry[$1].onPendingTexturesChanged(!!$0); },  
 320163: ($0, $1, $2) => { const canvas = self.WorkerGlobalScope ? new OffscreenCanvas(128, 128) : document.createElement('canvas'); const gl = canvas.getContext('webgl'); if (gl) { const di = gl.getExtension('WEBGL_debug_renderer_info'); if (di) { stringToUTF8(gl.getParameter(di.UNMASKED_RENDERER_WEBGL), $0, $2); stringToUTF8(gl.getParameter(di.UNMASKED_VENDOR_WEBGL), $1, $2); } } },  
 320525: () => { return !!GLctx },  
 320542: () => { debugger },  
 320555: ($0, $1, $2, $3, $4) => { window.inkHostRegistry[$0].onFatalError(UTF8ToString($1, $2), UTF8ToString($3, $4)); }
};
